...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for dev in hass.data[TPLINK_DOMAIN][CONF_SWITCH]:
        _LOGGER.debug("SWITCH DEV: {}".format(dev))
        if isinstance(dev, SmartPlug):
            devs.append(SmartPlugSwitch(dev))
        elif isinstance(dev, SmartStrip):
            # SmartStrip needs to be broken down into parent device and child plugs.
            # TODO: parent device and maybe wrap slots
            _LOGGER.debug("***** SmartStrip has {} plugs ***** ".format(len(dev.plugs)))
            for plug in dev.plugs:  # add each child plug in strip
                devs.append(SmartPlugSwitch(plug))
...
# class SmartStripSwitch(SmartPlugSwitch):
#     """Representation of a TPLink Smart Strip."""
#
#     def __init__(self, smartstrip):
#         """Initialize the switch."""
#         self.smartstrip = smartstrip
#         self._sysinfo = None
#         self._state = None
#         self._available = False
#         # Set up emeter cache
...
#
#
# class SmartStripSlotSwitch(SmartPlugSwitch):
#     """Representation of a TPLink Smart Strip Slot."""
#     pass
